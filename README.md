...
Exactement — et c'est une bonne pratique souvent oubliée. Voici comment le vérifier concrètement.

## La vérification mathématique

Un vecteur est L2-normalisé si et seulement si sa norme euclidienne vaut exactement 1 :

```
‖v‖ = √(v₁² + v₂² + ... + vₙ²) = 1.0
```

En pratique, à cause de la précision flottante, on accepte une tolérance : `|‖v‖ - 1.0| < 1e-6`.

## En Python, après récupération des chunksLe pattern de vérification est simple. En production avec numpy ou les SDKs Salesforce :

```python
import numpy as np

def verify_l2_normalization(embeddings: list[list[float]], tol: float = 1e-6) -> dict:
    """
    Vérifie a posteriori si les embeddings sortis du chunking sont L2-normalisés.
    Compatible avec les vecteurs retournés par Data Cloud, OpenAI, Cohere, etc.
    """
    arr = np.array(embeddings)
    norms = np.linalg.norm(arr, axis=1)          # ‖vᵢ‖ pour chaque chunk
    deltas = np.abs(norms - 1.0)
    is_normalized = deltas < tol

    return {
        "total_chunks"     : len(embeddings),
        "normalized_count" : int(is_normalized.sum()),
        "min_norm"         : float(norms.min()),
        "max_norm"         : float(norms.max()),
        "mean_norm"        : float(norms.mean()),
        "all_normalized"   : bool(is_normalized.all()),
        "metric_to_use"    : "dot_product" if is_normalized.all() else "cosine",
        "outliers"         : [i for i, ok in enumerate(is_normalized) if not ok]
    }
```

## Ce que les norms révèlent sur le modèle source

| Norme observée | Interprétation | Métrique |
|---|---|---|
| Toutes ≈ 1.0 (delta < 1e-6) | Modèle normalise par défaut (emb-3, Cohere, Salesforce interne) | dot product OK |
| Norms variables 0.5 – 4.0 | Ada-002 brut, modèles HuggingFace non patchés | cosine obligatoire |
| Norms corrélées à la longueur du chunk | Modèle encode la longueur dans la magnitude | cosine obligatoire |
| Toutes identiques mais ≠ 1 | Normalisation custom à une autre échelle | vérifier le pipeline |

## Dans le contexte Data Cloud spécifiquement

Après ingestion, tu peux interroger les chunks directement pour échantillonner leurs vecteurs et les tester :

```sql
-- Récupérer un échantillon de vecteurs pour vérification
SELECT chunk_id, chunk_number, embedding_vector
FROM EinsteinChunk__dlm
WHERE source_id IN (
    SELECT Id FROM KnowledgeArticle__dlm LIMIT 50
)
ORDER BY RAND()
LIMIT 200
```
